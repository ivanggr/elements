...
const bit BIT_MAX = 1;
const byte TETRA_MAX = 0xF;

/* размеры тетрады, байта, слова */
const size_t TETRA_SIZE_IN_BITS = 4;
const size_t BYTE_SIZE_IN_BITS = CHAR_BIT * sizeof(byte);
const size_t WORD_SIZE_IN_BITS = CHAR_BIT * sizeof(word);
const size_t BYTE_SIZE_IN_TETRAS = BYTE_SIZE_IN_BITS / TETRA_SIZE_IN_BITS + (BYTE_SIZE_IN_BITS % TETRA_SIZE_IN_BITS != 0 ? 1 : 0);
const size_t WORD_SIZE_IN_BYTES = WORD_SIZE_IN_BITS / BYTE_SIZE_IN_BITS;
...
const char ENDIAN_NAMES[][14] = {"Little Endian", "Big Endian"};

/* операции с битами */
const bit BIT_SET = 1;
const bit BIT_CLEAR = 0;
...
inline word form_word4(byte *byte_mas, endian_types src_endian_type);
