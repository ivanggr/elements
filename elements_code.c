...
const byte bit_in_word_erase_mask = 31;	// = 11111b; этой маской и логическим 'И' стираем все, кроме пяти разрядов

/* в составе байта 8 = 2^3 бита, для адресации бита в составе слова необходимо три разряда */
const byte bit_in_byte_erase_mask = 7; // = 111b; этой маской и логическим 'И' стираем все, кроме трех разрядов

/* массивы масок для выделения байт в составе слова */
const word BYTE_SHIFT[] = { 0*BYTE_SIZE_IN_BITS, 1*BYTE_SIZE_IN_BITS, \
	2*BYTE_SIZE_IN_BITS, 3*BYTE_SIZE_IN_BITS }; // на сколько разрядов нужно сдвинуть

const word BYTE_MASK[] = { UCHAR_MAX << BYTE_SHIFT[0], UCHAR_MAX << BYTE_SHIFT[1], \
...
{
        // игнорируем все кроме первых пяти разрядов, учитываем от ноля до тридацати одного	
        bit_number &= bit_in_word_erase_mask;
	// у бита стираем все разряды, кроме младшего
        bit_value &= 0x01;	
	w |= bit_value << bit_number;
	return w;
}
...
{
	bit_number &= bit_in_word_erase_mask;	// игнорируем все кроме первых пяти разрядов, учитываем от ноля до тридацати одного
	w &= 0x00000001 << bit_number;
	w >>= bit_number;
	return w == 1 ? 1 : 0;
}
...
{
	bit_number &= bit_in_byte_erase_mask;	// игнорируем все кроме первых трех разрядов, учитываем от ноля до семи
	bit_value &= 0x01;	// у бита стираем все разряды, кроме младшего
	b |= bit_value << bit_number;
	return b;
}
...
inline word word_BIG_ENDIAN4(byte b0, byte b1, byte b2, byte b3)
{
...
inline word word_LITTLE_ENDIAN4(byte b0, byte b1, byte b2, byte b3)
{
...
{
	tetra &= 0xF;	// оставляем только младшие четыре разряда
	return tetra >= 0xA ? ('A' + ((char) (tetra - 0xA))) : ('0' + ((char) tetra));
}
